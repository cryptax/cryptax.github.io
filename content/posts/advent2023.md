...
# December 12 - Hot Springs

## 12.1

We have a map of operational springs `.` and damaged springs `#`.
Except sometimes the map is incomplete and we have a `?` which means we are not sure the spring is operational or not.

To help us we have a correct count of contiguous damaged springs. 
For example `???.### 1,1,3` tells us we have one damaged spring, seperated another damaged spring, and then seperated 3 contiguous damaged springs. In that case, there's only one good solution: `#.#.###`.
We need to count the number of solutions per line.

I process lines one at a time.
I seperate each line in what I call "groups" and "lens". I could have called "groups" rather "map" and "lens" something like damaged count.

```python
    g, l = line.strip().split(' ')
    lens = [int(i) for i in l.split(',') ]
```

> I could have used `map`: `lens = list(map(int, l.split(',')))`

I rule out the simple case where we have no `?`:
...
Then we are in a configuration where we have `#`, `?` and counts of damaged springs. I know how many `?` are actually damaged by computed `sum(lens) - g.count('#')`, but I don't know which ones.

My idea was to generate all possible combinations. I get the positions of all `?` and must generate combinations where the right amount of those questions marks are turned into hashes.

```python
    # get position of all jokers
    joker_pos = [index for index, value in enumerate(g) if value=='?']
    logger.debug(f'Position of Jokers={joker_pos}')
    # generate possibilities of positions we must replace ? by #
    possible_pos = list(combinations(joker_pos, count_new))
```

This uses `combinations` which is from `itertools`:

```
from itertools import combinations
```

Then I check each combination and check if they are possible. My combinations work over position, I need to create the corresponding map:
...
To check a line is possible, I implement `corresponds`. Note that my `groups` will only have `.` and `#` but no question marks. The test is rather simple, I need to verify I have the correct amount of groups, and then count the length of each group is correct.

```python
def corresponds(groups: str, lens: list[int]) -> bool:
    groups_tab = list(filter(None, groups.split('.'))) # remove empty groups
    if len(groups_tab) != len(lens):
        return False

    for index in range(0, len(lens)):
        if groups_tab[index].count('#') != lens[index]:
            return False

    return True
```

If the line I generated by modifying some `?` is possible, I increment the number of arrangements:

```python
        if corresponds(''.join(generated), lens):
            logger.debug(f"{''.join(generated)} matches {lens}")
            arrangements = arrangements + 1
```

My code worked, and from what I read only few people did it with combinations. However, this was not a good solution for task 2 where the groups were longer: combinations exploded.
