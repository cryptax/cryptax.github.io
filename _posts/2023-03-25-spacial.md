...
1. Get the current timestamp and create a new OTP code based on the timestamp.
...
2. Tranform the 6-digit OTP code in a 6-digit code using only numbers between 0 and 3.
...
3. Compare the input sequence with the expected one. Display the ACCESS GRANTED image and the flag (from the EEPROM) if the code is correct. 
