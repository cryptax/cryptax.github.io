...
# Welcome - Insomni'hack CTF challenge
...
**To be honest, I was more concerned by the first point than by the second**, because I thought it would be used to convey the flag (we're in a CTF after all) than the first which allows more stealthy actions.
...
The flag is long, so I decide to do some sed on the output to retrieve the flag: 
...
3. Pull and run `welcome_inso23-1`
...
The script gives a link to [this page](https://zwischenzugs.com/2015/06/24/the-most-pointless-docker-command-ever/) which explains how to run a Docker container will full root capabilities, using `--privileged`, `--net=host`, `--pid=host`, `--ipc=host` and mounting `/`.
...
I am pretty sure all Anti-Virus analysts will back me on this one.
